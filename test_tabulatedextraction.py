'''testing the extraction of plate ids and all necesaary info indicated by stephen from the logbooks'''
...
'''test a logbook and see if it can give back a tabulated df of all the info'''

'''before that, im trying to make a semantic meaning visualization with network x and maybe a summary text analysis as well
...
def main():
    ...
    # give scraper param ot only extract and download folders with format "observation notes" and "photographs"
        ...
        if filename.endswith('.pdf'):
            ...
            for page_num in range(pdf.page_count):
                ...
                try:
                    ...
                    # Process page content (integrate with your ChatGPT API logic)
    ...
    # Load mock data
    # mock_data = load_mock_output(mock_file_path)
    ...
    for record in mock_data:
        ...
        # catalog_meta = get_metadata_for_catalog(identifier)
        catalog_meta = scraper.get_metadata_by_id(identifier)
        # print(catalog_meta)

        # Insert into database
        # sql.insert_document(db_path, processed_docs, catalog_meta)
        # for batch in get_docs(download_folder):
    # Remove duplicates
    sql.delete_duplicates(db_path)
