...
def process_pdf(file_content, catalog_metadata, limit=1):
    ...
    for page_num in range(start_page, pdf.page_count):
        ...
        Your goal is to provide accurate extraction of all the information in these tables and from the overall document. Focus on:
        - Accurate tabular data extraction, ensuring fields are mapped to the following columns:
        - Plate ID (NEG. NO), Guide Star, Right Ascension (R.A.), Declination (DEC), Date, H.A. END, Exposure MST AM-PM, INST PLATE, TELE POSITION, DEVELOPMENT (Kind, Time, Temp), Seeing, and Remarks.
        - Handling multi-line rows and irregular alignments.
        - Correctly parsing numeric, symbolic, and abbreviated data formats.
        If you encounter inconsistent or unstructured data, include those rows under a "row_errors" key in the JSON output for further review. 
        """

        prompt_content = '''Extract the tabular data, paying close attention to:
- All columns such as NEG. NO (Plate ID), REGION (Guide Star, Right Ascension, Declination), DATE, H.A. END, EXPOSURE MST AM-PM, INSTRUMENT (INST), PLATE, TELE POSITION, DEVELOPMENT (Kind, Time, Temp), SEEING (trans, stead., obsv.), and REMARKS.
- For rows with missing data, provide empty strings as placeholders for the respective fields.
- Combine multi-line entries into single rows if they belong to the same observation.
- Ensure fields are mapped correctly to their respective headers.
'''
  ...
  "schema": {
      ...
      "tabular_data": {
        ...
        "items": {
          ...
          "properties": {
            ...
            "right_ascension": {"type": "string", "default": ""},
            "declination": {"type": "string", "default": ""},
            "date": {"type": "string", "default": ""},
            "ha_end": {"type": "string", "default": ""},
            "exposure_mst_am_pm": {"type": "string", "default": ""},
            "we": {"type": "string", "default": ""},
            "instrument": {"type": "string", "default": ""},
            "plate": {"type": "string", "default": ""},
            "tele_position": {"type": "string", "default": ""},
            "development_kind": {"type": "string", "default": ""},
            "development_time": {"type": "string", "default": ""},
            "development_temp": {"type": "string", "default": ""},
            "seeing_trans": {"type": "string", "default": ""},
            "seeing_stead": {"type": "string", "default": ""},
            "seeing_obs": {"type": "string", "default": ""},
    ...
    "required": ["tabular_data", "metadata"],
    "additionalProperties": False
  }
}
        ...
        extracted_data.append({
            ...
            'row_errors': parsed_output.get("row_errors", ""),
...
if __name__ == "__main__":
    ...
    context = None  # Context inot needed rn 
    ...
    if response["statusCode"] != 200:
        ...
        for entry in processed_data:
            ...
            for record in entry.get("tabular_data", []):
                row = {
                    "catalog_id": catalog_id,
                    "page_num": page_num,
